...
def test_import_without_pydantic(monkeypatch):
    ...
    sys.modules.pop('brazilian.pydantic.cpf_model', None)
    monkeypatch.setitem(sys.modules, 'brazilian.pydantic.cpf_model', None)

    sys.modules.pop('brazilian.sqlalchemy.cpf_type', None)
    module = importlib.import_module("brazilian.sqlalchemy.cpf_type")

    assert module._PYDANTIC_AVAILABLE is False
    assert module.CPFModel is None