...
def test_import_without_pydantic(monkeypatch):
    ...
    sys.modules.pop('brazilian.pydantic.time_model', None)
    monkeypatch.setitem(sys.modules, 'brazilian.pydantic.time_model', None)

    sys.modules.pop('brazilian.sqlalchemy.time_type', None)
    module = importlib.import_module("brazilian.sqlalchemy.time_type")

    assert module._PYDANTIC_AVAILABLE is False
    assert module.TimeModel is None